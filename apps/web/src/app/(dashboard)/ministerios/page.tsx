      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {ministerios.map((m) => {
            ...
            return (
              <EntityCard key={m.id} className={`flex flex-col justify-between relative overflow-hidden transition-all hover:shadow-md ${!isAtivo ? 'opacity-65' : ''}`}>
                <div className="absolute top-4 right-4 flex flex-col items-end gap-1.5">
                  <StatusBadge
                    label={isAtivo ? t('status.active') : t('status.archived')}
                    className={`border ${isAtivo ? 'bg-indigo-50 text-indigo-700 border-indigo-100' : 'bg-gray-100 text-gray-500 border-gray-200'}`}
                  />
