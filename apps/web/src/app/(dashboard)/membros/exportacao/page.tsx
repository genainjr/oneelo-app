...
export default function MembrosExportacaoPage() {
  ...
  function handleExport() {
    ...
    const rows = membros.map((m) =>
      active.map((f) => {
        if (f.key === 'tags') return m.tags.map((t) => t.tag.nome).join('; ');
    ...
    downloadCsv(`membros-${new Date().toISOString().slice(0, 10)}.csv`, headers, rows);
  }
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-6">
          ...
          {loading ? (
            ...
            <>
              ...
              <span className="text-sm text-indigo-700">
                <strong>{membros.length}</strong> membros · <strong>{selectedFields.length}</strong> campo(s) selecionado(s)
              </span>
            </>
          )}
        </div>
        ...
        <button
          ...
          disabled={loading || selectedFields.length === 0 || membros.length === 0}
