  ...
  function handleExport() {
    ...
    const rows = eventos.map((ev) =>
      active.map((f) => {
        ...
        return (ev as any)[f.key] ?? '';
      }),
    );
    downloadCsv(`agenda-${new Date().toISOString().slice(0, 10)}.csv`, headers, rows);
  }
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-6">
          ...
          {loading ? (
            ...
            <>
              ...
              <span className="text-sm text-indigo-700">
                <strong>{eventos.length}</strong> evento(s) · <strong>{selectedFields.length}</strong> campo(s) selecionado(s)
              </span>
            </>
          )}
        </div>
        ...
        <button
          ...
          disabled={loading || selectedFields.length === 0 || eventos.length === 0}
