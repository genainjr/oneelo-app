...
export default function EscalasExportacaoPage() {
  ...
  function handleExport() {
    ...
    downloadCsv(`escalas-${new Date().toISOString().slice(0, 10)}.csv`, headers, rows);
  }
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-6">
          ...
          {loading ? (
            ...
            <>
              ...
              <span className="text-sm text-indigo-700">
                <strong>{escalas.length}</strong> escala(s) · <strong>{selectedFields.length}</strong> campo(s) selecionado(s)
              </span>
            </>
          )}
        </div>
        ...
        <button
          ...
          disabled={loading || selectedFields.length === 0 || escalas.length === 0}
