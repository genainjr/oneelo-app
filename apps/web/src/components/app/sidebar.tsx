      ...
      >
        ...
        <nav className="flex-1 px-2 py-4 space-y-0.5 overflow-y-auto overflow-x-hidden">
          ...
          {visibleItems.map((item) => {
            ...
            if (hasChildren) {
              const sectionActive = pathname.startsWith('/ministerios');
              return (
                <div key={item.href}>
                  {/* Expandido: toggle puro (sem Link) */}
                  {!collapsed && (
                    <button
                      onClick={() => setMinisteriosOpen((v) => !v)}
                      title={ministeriosOpen ? 'Recolher módulos' : 'Expandir módulos'}
                      className={cn(
                    ...
                    >
                      ...
                      <svg
                        className={cn('w-3.5 h-3.5 flex-shrink-0 transition-transform duration-200', ministeriosOpen ? 'rotate-180' : '')}
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                        strokeWidth={2.5}
                  ...
                  {/* Colapsado: Link direto para /ministerios (accordion invisível) */}
                  {collapsed && (
                    <Link
                      href={item.href}
                      onClick={onClose}
